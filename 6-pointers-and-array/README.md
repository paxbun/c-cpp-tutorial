...
그런데, "expression must be a modifiable lvalue", 즉 수정 가능한 좌측값이 아니라는 오류가 표시됩니다. `arr[0]`이나 `arr[1]`은 좌측값인데, 왜 `arr`은 좌측값이 아닐까요? 나중에 다루겠지만, `arr`은 좌측값이 맞습니다. `arr`에 주소 연산자를 사용하여 `&arr` 처럼 쓸 수 있습니다. 하지만 배열이 좌측값으로 동작할 수 있는 경우는
