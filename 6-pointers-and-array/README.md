...
배열의 길이를 표시할 때는, 첨자 연산자와 달리 `[]` 안에 들어있는 값이 항상 컴파일할 때 결과값을 낼 수 있는 식이어야 합니다.
```c
int main()
{
    int arr[1 + 2];
...
> 변수가 대응된 주소는 여러 요인에 의해 바뀝니다. 위 출력 결과는 예시 출력으로, 아마 프로그램을 실행할 때 마다 바뀔 것입니다. 이것이 어떻게 바뀌게 되는지는 나중에 다룰 예정입니다.
...
그런데 여러분이 지금까지 워드나 크롬같은 프로그램을 사용하시면서 직접 주소를 입력해야했던 적이 있나요? 없을 것이라고 생각합니다. 그래서 이 기능은 여러분이 앞으로 쓸 일이 없을 것이라고 생각합니다.