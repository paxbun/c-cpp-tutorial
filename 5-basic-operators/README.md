...
```
중괄호를 아래에 붙이는 것은 마이크로소프트의 스타일입니다. 또 전 들여쓰기를 4칸으로 썼지만, 구글에선 2칸으로 씁니다.
```c
#include <stdio.h>
int main() {
  printf("Hello, world!");
...
```
들여쓰기를 4칸으로 하는 것도 마이크로소프트의 스타일입니다. 제가 마이크로소프트의 스타일을 따라가는 경향이 있지만, 여러분은 제 스타일에 굳이 맞출 필요는 없고, 여러분에게 맞는 스타일을 찾으시면 됩니다. 여러 코딩 스타일들을 보고 싶으시다면, 이 [위키피디아 문서](https://en.wikipedia.org/wiki/Indentation_style)를 참고해주세요. 하지만 어떤 스타일이든 다른 사람들이 읽기 편하게 정렬하는 것이 가장 좋습니다. 여러분이 어떤 회사나 기관에서 일하게 된다면, 그 곳의 코딩 스타일을 따라야 합니다.