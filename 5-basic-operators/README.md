...
이렇게 분리될 수 있습니다. 여기서 알 수 있는 것은 띄어쓰기나 줄바꿈은 이 토큰들을 구분하기 위한 용도로만 쓰인다는 것입니다. 지금 토큰이 정확히 어떻게 분리되는지 외울 필요는 없습니다. 문법을 차차 배워가면서 이 규칙을 체득할 수 있습니다.

이 토큰들을 어떻게 정렬하느냐에 따라 그 사람의 코딩 스타일이 달라집니다. 전 지금까지 중괄호`{`를 `int main()`의 아래에 붙였지만, 옆에 붙이는 것이 더 일반적입니다.
```c
int main() {
    int i = 0;
}
...
```
들여쓰기를 4칸으로 하는 것도 마이크로소프트의 스타일입니다. 제가 C\# 스타일을 따라가는 경향이 있지만, 여러분은 제 스타일에 굳이 맞출 필요는 없고, 여러분에게 맞는 스타일을 찾으시면 됩니다. 여러 코딩 스타일들을 보고 싶으시다면, 이 [위키피디아 문서](https://en.wikipedia.org/wiki/Indentation_style)를 참고해주세요. 하지만 어떤 스타일이든 다른 사람들이 읽기 편하게 정렬하는 것이 가장 좋습니다. 여러분이 어떤 회사나 기관에서 일하게 된다면, 그 곳의 코딩 스타일을 따라야 합니다.