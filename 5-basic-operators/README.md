...
이렇게 변수에 저장되지는 않았지만 식이 들어가는 자리에 바로 넣은 값을 `리터럴`이라고 부릅니다. 위의 예시는 모두 10진수 리터럴인데요, 2진수, 8진수, 16진수 리터럴도 있습니다.
...
18 20 564
```
이렇게 `0b`로 시작하는 숫자는 2진수, `0`으로 시작하는 숫자는 8진수, `0x`으로 시작하는 숫자는 16진수가 됩니다. 여기에 위에서 다뤘던 것 처럼 `f`나 `l` 같은 것을 붙여 자료형을 지정할 수 있습니다. 지금은 숫자의 리터럴만을 다뤘지만, 나중에 다른 종류의 리터럴도 나올 예정입니다.
...
!["Narrowing conversion warning"](img/5.png "Narrowing conversion warning")

이렇게 오류는 아니지만 경고를 띄워줍니다. `경고(warning)`라는 건 코드를 컴파일할 수는 있지만 코드가 좋지 않음을 알려주는 기능입니다. `오류(error)`가 발생하면 항상 코드를 컴파일할 수 없는 것과는 차이가 있습니다. 그런데 컴파일러의 기능 중에는 모든 경고를 오류로 취급하는 기능이 있습니다. 그래서 이 기능을 함께 사용하면 모든 암시적 축소 변환을 막을 수 있습니다. 이렇게 하면 축소 변환이 필요할 때, 프로그래머가 항상 명시적으로 표시하도록 강제할 수 있습니다.
...
```
이 다음에 단어를 형태소나 접사로 분리하는 과정이 있겠죠.
